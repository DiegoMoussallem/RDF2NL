package org.aksw.rdf2fr.triple2nl;
///*
// * #%L
// * Triple2NL
// * %%
...
import org.aksw.rdf2fr.triple2nl.converter.DefaultIRIConverterFrench;
import org.aksw.rdf2fr.triple2nl.converter.IRIConverter;
import org.aksw.rdf2fr.triple2nl.converter.LiteralConverterFrench;
import org.aksw.rdf2fr.triple2nl.gender.DictionaryBasedGenderDetector;
import org.aksw.rdf2fr.triple2nl.gender.Gender;
import org.aksw.rdf2fr.triple2nl.gender.GenderDetector;
import org.aksw.rdf2fr.triple2nl.nlp.stemming.PlingStemmer;
import org.aksw.rdf2fr.triple2nl.property.PropertyVerbalization;
import org.aksw.rdf2fr.triple2nl.property.PropertyVerbalizationType;
import org.aksw.rdf2fr.triple2nl.property.PropertyVerbalizerFrench;
import org.aksw.rdf2fr.triple2nl.util.GenericType;
import org.aksw.rdf2fr.triple2nl.util.Sparql;
import org.aksw.rdf2fr.utils.nlp.pos.TreeTagger;
 ...
 * 
 */
public class TripleConverterFrench {
	
	private static final Logger logger = LoggerFactory.getLogger(TripleConverterFrench.class);

	private static String DEFAULT_CACHE_BASE_DIR = System.getProperty("java.io.tmpdir");
	private static String DEFAULT_CACHE_DIR = DEFAULT_CACHE_BASE_DIR + "/triple2nl-cache/french";

	private NLGFactory nlgFactory;
	private Realiser realiser;

	private IRIConverter uriConverter;
	private LiteralConverterFrench literalConverter;
	private PropertyVerbalizerFrench pp;
	...
	public TripleConverterFrench() {
		this(new QueryExecutionFactoryModel(ModelFactory.createDefaultModel()), DEFAULT_CACHE_DIR, new XMLLexicon());
	}

	public TripleConverterFrench(SparqlEndpoint endpoint) {
		this(endpoint, DEFAULT_CACHE_DIR);
	}
	
	public TripleConverterFrench(QueryExecutionFactory qef, String cacheDirectory, Dictionary wordnetDirectory) {
		this(qef, null, null, cacheDirectory, wordnetDirectory, null);
	}
	
	public TripleConverterFrench(SparqlEndpoint endpoint, String cacheDirectory) {
		this(endpoint, cacheDirectory, null);
	}
	
	public TripleConverterFrench(SparqlEndpoint endpoint, String cacheDirectory, Dictionary wordnetDirectory) {
		this(new QueryExecutionFactoryHttp(endpoint.getURL().toString(), endpoint.getDefaultGraphURIs()), 
				null, null, cacheDirectory, wordnetDirectory, new XMLLexicon());
	}
	
	public TripleConverterFrench(SparqlEndpoint endpoint, String cacheDirectory, Dictionary wordnetDirectory, Lexicon lexicon) {
	...
	public TripleConverterFrench(QueryExecutionFactory qef, IRIConverter uriConverter, String cacheDirectory, Dictionary wordnetDirectory) {
		this(qef, null, uriConverter, cacheDirectory, wordnetDirectory, new XMLLexicon());
	}
	
	public TripleConverterFrench(QueryExecutionFactory qef, String cacheDirectory, Lexicon lexicon) {
		this(qef, null, null, cacheDirectory, null, lexicon);
	}
	
	public TripleConverterFrench(QueryExecutionFactory qef, PropertyVerbalizerFrench propertyVerbalizer, IRIConverter uriConverter, String cacheDirectory, Dictionary wordnetDirectory, Lexicon lexicon) {
		if(uriConverter == null){
			uriConverter = new DefaultIRIConverterFrench(qef, cacheDirectory);
		}
		this.uriConverter = uriConverter;
		
		if(propertyVerbalizer == null){
			propertyVerbalizer = new PropertyVerbalizerFrench(uriConverter, wordnetDirectory);
		}
		...
		literalConverter = new LiteralConverterFrench(uriConverter);
	...
	public String convert(List<Triple> triples){
		...
		List<SPhraseSpec> typePhrases = convertToPhrases(typeTriples, false);
		...
		if(typePhrases.size() > 1){
			...
			if(useAsWellAsCoordination){
				...
				combinedLastTwoObjects.setConjunction("ainsi que");
				combinedLastTwoObjects.setFeature(Feature.RAISE_SPECIFIER, false);
				combinedLastTwoObjects.setFeature(InternalFeature.SPECIFIER, "un"); //fix
		...
		List<SPhraseSpec> otherPhrases = convertToPhrases(otherTriples,false);
	 ...
	 * @return the phrase
	 */
	public SPhraseSpec convertToPhrase(Triple t, boolean dead) {
		return convertToPhrase(t, false, dead);
	}
	 ...
	 * @return the phrase
	 */
	public SPhraseSpec convertToPhrase(Triple t, boolean negated, boolean dead) {
		return convertToPhrase(t, negated, false, dead);
	}
	 ...
	 * @return the phrase
	 */
	public SPhraseSpec convertToPhrase(Triple t, boolean negated, boolean reverse, boolean dead) {
...
//			logger.info("Predicate is Variable");
...
//		else {
			logger.debug("Predicate is not Variable");
			...
			if (objectIsClass) {
				...
				p.setVerb("être");
				if(dead == true){ // in case if this a dead person, set PAST as a tense
					//logger.info("true");
				...
				if (genericType.equals("http://dbpedia.org/ontology/Person")){
					if (gender.name().equals("FEMALE")) {
						objectElement.setSpecifier("un");
					} else { objectElement.setSpecifier("un");
					}
				}
...
//					subjectElement = nlgFactory.createNounPhrase(subjectWord);
					...
					String predicado;
					try {
						treetagger = new TreeTagger(language);
	                ...
	                //predicateNounPhrase.addComplement("de");
	                predicateNounPhrase.setFeature(InternalFeature.POSTMODIFIERS, subjectElement); //insert the subject as possessive like "dele" and "dela"
					...
					System.out.println("Predicado: " + predicado + "Primeiro token: " + input.get(0).toString() + "Tag: " + tag.toString());
					
					if(tag.contains("NCM")){
						predicateNounPhrase.setSpecifier("o");	
					} else if(tag.contains("NCF")) {
						predicateNounPhrase.setSpecifier("a");
					} else { predicateNounPhrase.setSpecifier("o");
						};
...
//				else {
//
//					List<org.aksw.rdf2nl.triple2nl.nlp.relation.Pattern> l = BoaPatternSelector
...
//		System.out.println(realiser.realise(p));
	 ...
	 */
	public List<SPhraseSpec> convertToPhrases(Collection<Triple> triples, boolean dead) {
		List<SPhraseSpec> phrases = new ArrayList<>();
		for (Triple triple : triples) {
			phrases.add(convertToPhrase(triple, dead));
		}
		return phrases;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(new TripleConverterFrench().convert(
				Triple.create(
