...
import org.aksw.rdf2fr.triple2nl.converter.URIDereferencer.DereferencingFailedException;
 ...
 *
 */
public class DefaultIRIConverterFrench implements IRIConverter {

	private static final Logger logger = LoggerFactory.getLogger(DefaultIRIConverterFrench.class);
	...
	public DefaultIRIConverterFrench(SparqlEndpoint endpoint, String cacheDirectory) {
		this(new QueryExecutionFactoryHttp(endpoint.getURL().toString(), endpoint.getDefaultGraphURIs()),
				cacheDirectory);
	}

	public DefaultIRIConverterFrench(SparqlEndpoint endpoint) {
		this(endpoint, null);
	}

	public DefaultIRIConverterFrench(QueryExecutionFactory qef) {
		this(qef, null);
	}

	public DefaultIRIConverterFrench(QueryExecutionFactory qef, String cacheDirectory) {
		this.qef = qef;

		// use tmp as default cache directory
		if (cacheDirectory == null) {
			cacheDirectory = System.getProperty("java.io.tmpdir") + "/triple2nl/cache/french";
		}
	...
	public DefaultIRIConverterFrench(Model model) {
		this(new QueryExecutionFactoryModel(model));
	}

	public DefaultIRIConverterFrench(OWLOntology ontology) {
	...
	public static void main(String[] args) throws MalformedURLException {
		...
		DefaultIRIConverterFrench converter = new DefaultIRIConverterFrench(
				SparqlEndpoint.create("http://fr.dbpedia.org/sparql", ""));
